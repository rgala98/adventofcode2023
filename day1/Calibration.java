...
public class Calibration {
    ...
    public static int sumOfCalibrationValues(String[] lines) {
        int sum = 0;

        for (String line : lines) {
            int calibrationValue = extractFirstAndLastDigit(line);
    ...
    private static int extractFirstAndLastDigit(String line){
        ...
        HashMap<String, Integer> digitValues = new HashMap<>();
        ...
        for(int i=0; i<line.length();i++){
            ...
            char lc = line.charAt(j);
            if(line.length() >= 3){
                if(i+3 < line.length()){
                  String threechars = line.substring(i,i+3);
                    if(!fcFound && digitValues.containsKey(threechars)){
                        firstDigit = digitValues.get(threechars);
                        fcFound = true;
                    }  
                }
                
                if(i+4 < line.length()){
                   String fourchars = line.substring(i,i+4);
                    if(!fcFound && digitValues.containsKey(fourchars)){
                        firstDigit = digitValues.get(fourchars);
                        fcFound = true;
                    } 
                }
                
                if(i+5 < line.length()){
                   String fivechars = line.substring(i,i+5);
                    if(!fcFound && digitValues.containsKey(fivechars)){
                        firstDigit = digitValues.get(fivechars);
                        fcFound = true;
                    } 
                }
                ...
                if(j-3 > -1){
                    String chars3 = line.substring(j-2,j+1);
                    
                    if(!lcFound && digitValues.containsKey(chars3)){
                        lastDigit = digitValues.get(chars3);
                        lcFound = true;
                    } 
                }
                if(j-4 > -1){
                    String chars4 = line.substring(j-3,j+1);
                    
                    if(!lcFound && digitValues.containsKey(chars4)){
                        lastDigit = digitValues.get(chars4);
                        lcFound = true;
                    }
                }
                
                if(j-5 > -1){
                    String chars5 = line.substring(j-4,j+1);
                    if(!lcFound && digitValues.containsKey(chars5)){
                        lastDigit = digitValues.get(chars5);
                        lcFound = true;
                    }  
                }
            ...
            if(!fcFound && Character.isDigit(fc)){
                firstDigit = Character.getNumericValue(fc);
                fcFound = true;
            }
            if(!lcFound && Character.isDigit(lc)){
                lastDigit = Character.getNumericValue(lc);
                lcFound = true;
            }
...
"fourzqlhcjksixthreejrl9",
        ...
        int result = sumOfCalibrationValues(lines);
