...
public class EngineSchematic {
    ...
    public static void main(String[] args) {
        ...
        int sum = calculatePartNumbersSum(schematic);
    ...
    public static int calculatePartNumbersSum(String[] schematic) {
        ...
        Map<String, String> starAdjValues = new HashMap<>();

        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                char current = schematic[i].charAt(j);

                if (Character.isDigit(current)) {
                    boolean adjacentToSymbol = false;
                    String symbolIndex = "0,0";
                    ...
                    for (int k = 0; k < 8; k++) {
                        ...
                        if (x >= 0 && x < rows && y >= 0 && y < cols) {
                            char adjacentCell = schematic[x].charAt(y);
                            // Part 1
                            // if(!Character.isDigit(adjacentCell) && adjacentCell != '.')
                            if (adjacentCell == '*') {
                                symbolIndex = x + "," + y;
                    ...
                    if (adjacentToSymbol) {
                        String extractedNumber = extractNumber(schematic[i], j);
                        String gearRatios = starAdjValues.get(symbolIndex);

                        if (gearRatios != null) {
                            gearRatios += "*" + extractedNumber.split(";")[0];
                        } else {
                            gearRatios = extractedNumber.split(";")[0];
                        }
                        starAdjValues.put(symbolIndex, gearRatios);

                        sum += Integer.parseInt(extractedNumber.split(";")[0]);
                        j = Integer.parseInt(extractedNumber.split(";")[1]);
                    }
                }
            }
        }

        for (String key : starAdjValues.keySet()) {
            String[] multipliers = starAdjValues.get(key).split("\\*");
            int mul = 0;
            if (multipliers.length >= 2) {
                mul = Integer.parseInt(multipliers[0]) * Integer.parseInt(multipliers[1]);
                gearRatiosSum += mul;
            }
    ...
    public static String extractNumber(String input, int index) {
        int start = index;
        int end = index;
        ...
        return number + ";" + String.valueOf(end);
    }
}
