...
public class PossibleGames {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Integer> possibleGameConfig = new HashMap<>();
        possibleGameConfig.put("red", 12);
        possibleGameConfig.put("green", 13);
        possibleGameConfig.put("blue", 14);

        List<Integer> possibleGames = getPossibleGames(gamesInput, possibleGameConfig);

        int sum = 0;
        for (int game : possibleGames) {
            sum += game;
        }

        System.out.println("All Games: " + sum);
    }

    public static List<Integer> getPossibleGames(String[] gamesInput, Map<String, Integer> possibleGameConfig) {
        List<Integer> allGames = new ArrayList<>();
        for (String game : gamesInput) {
            String gameId = game.split(": ")[0].split(" ")[1];
            String pGames = game.split(": ")[1];
            allGames.add(getPowerOfMinimumSetCubes(pGames));
        }
        return allGames;
    }

    public static boolean isGamePossible(String game, Map<String, Integer> possibleGameConfig) {
        boolean isPossible = true;
        String[] subGames = game.split("; ");
        for (String subGame : subGames) {
            String[] cubes = subGame.split(", ");
            for (String cubeType : cubes) {
                String[] cube = cubeType.split(" ");
                if (Integer.parseInt(cube[0]) > possibleGameConfig.get(cube[1])) {
                    isPossible = false;
                }
            }
        }
    ...
    public static int getPowerOfMinimumSetCubes(String game) {
        String[] subGames = game.split("; ");
        ...
        for (String subGame : subGames) {
            ...
            for (String cubeType : cubes) {
                String[] cube = cubeType.split(" ");
                if (Integer.parseInt(cube[0]) > minSet.get(cube[1])) {
                    minSet.put(cube[1], Integer.parseInt(cube[0]));
                }
            }
        }
        int product = 1;
        for (int value : minSet.values()) {
            product *= value;
        }
        return product;
    }
