...
function getPossibleGames(gamesInput, possibleGameConfig) {
  allGames = [];
  gamesInput.forEach((game) => {
    const gameId = game.split(": ")[0].split(" ")[1];
    const pGames = game.split(": ")[1];
    // Part 1
    // if(isGamePossible(pGames, possibleGameConfig)){
    //     console.log("GAME POSSIBLE", gameId);
    //     allGames.push(
    //         gameId
    //     );
    // }
    ...
    // Part 2
    // console.log("Min set for", gameId, getPowerOfMinimumSetCubes(pGames))
    allGames.push(getPowerOfMinimumSetCubes(pGames));
  });
  return allGames;
}

function isGamePossible(game, possibleGameConfig) {
  let isPossible = true;
  const subGames = game.split("; ");
  subGames.forEach((subGame) => {
    const cubes = subGame.split(", ");
    cubes.forEach((cubeType) => {
      const cube = cubeType.split(" ");
      if (Number(cube[0]) > possibleGameConfig[cube[1]]) {
        isPossible = false;
      }
    });
  });
...
function getPowerOfMinimumSetCubes(game) {
  const subGames = game.split("; ");
  const minSet = {
    red: 0,
    green: 0,
  ...
  subGames.forEach((subGame) => {
    ...
    cubes.forEach((cubeType) => {
      const cube = cubeType.split(" ");
      if (Number(cube[0]) > minSet[cube[1]]) {
        minSet[cube[1]] = Number(cube[0]);
      }
    });
  });
  return Object.values(minSet).reduce((a, b) => Number(a) * Number(b));
}
...
// const possibleGames = getPossibleGames(gamesInput, possibleGameConfig);
const possibleGames = getPossibleGames(gamesInput, possibleGameConfig);

const sum = possibleGames.reduce((a, b) => Number(a) + Number(b), 0);

console.log("All Games", sum);
